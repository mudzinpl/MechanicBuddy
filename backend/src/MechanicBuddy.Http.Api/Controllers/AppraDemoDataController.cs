...
namespace MechanicBuddy.Http.Api.Controllers
{
    ...
    public class AppraDemoDataController : ControllerBase
    {
        ...
        public AppraDemoDataController(ISession session, IWebHostEnvironment environment, IOptions<DbOptions> dbOptions)
        {
        ...
        public ActionResult<AppraDemoDataStatusDto> GetStatus()
        {
            ...
            {
                Allowed = IsAllowedEnvironment(),
                Exists = DemoExists(),
                WorkCount = DemoWorkCount(),
                Message = IsAllowedEnvironment()
        ...
        public ActionResult<AppraDemoDataResultDto> Create([FromBody] AppraDemoDataRequestDto request)
        {
            ...
            var vehicleIds = CreateVehicles(clientIds);
            var replacementVehicleIds = CreateReplacementVehicles();
        ...
        private bool IsAllowedEnvironment()
        {
            ...
            return environment.IsDevelopment()
                ...
                || dbOptions.MultiTenancy?.Enabled == false;
        }
        ...
        private DemoEmployees EnsureDemoEmployees()
        {
            ...
            var adminId = currentEmployeeId ?? session.Connection.ExecuteScalar<Guid?>("SELECT id FROM domain.employee ORDER BY introducedat LIMIT 1") ?? Guid.NewGuid();
            ...
            {
                var existingId = session.Connection.ExecuteScalar<Guid?>(
                    "SELECT id FROM domain.employee WHERE email = @Email",
                    new { employee.Email });
            ...
            return new DemoEmployees(adminId, ids[0], ids[1], ids[2], ids[3], ids[4]);
        }
        ...
        private List<Guid> CreateClients()
        {
            ...
            for (var i = 0; i < clients.Length; i++)
            {
                ...
                var emailName = (client.CompanyName ?? $"{client.FirstName}.{client.LastName}").ToLowerInvariant().Replace(" ", ".").Replace("ł", "l").Replace("ó", "o");
                session.Connection.Execute("INSERT INTO domain.clientemail(clientid, address, isactive) VALUES (@ClientId, @Address, true)",
                    new { ClientId = id, Address = $"{emailName}@appra-demo.local" });
                ids.Add(id);
            }
        ...
        private List<Guid> CreateVehicles(List<Guid> clients)
        {
            var vehicles = new[]
            {
            ...
            for (var i = 0; i < vehicles.Length; i++)
            {
                ...
                session.Connection.Execute(@"
                    ...
                    VALUES (@Id, @Producer, @Model, @RegNr, @Vin, @Odo, @Body, @Engine, @ProductionDate, @Description, false, CURRENT_TIMESTAMP)",
                    new
                    {
                        Id = id,
                        vehicle.Producer,
                        ...
                        vehicle.Vin,
                        vehicle.Odo,
                        Body = vehicle.Model.Contains("Vito") || vehicle.Model.Contains("Master") ? "Dostawczy" : "Osobowy",
                        Engine = "benzyna/diesel",
        ...
        private List<Guid> CreateReplacementVehicles()
        {
            var vehicles = new[]
            {
            ...
            foreach (var vehicle in vehicles)
            {
                ...
                session.Connection.Execute(@"
                    INSERT INTO domain.vehicle(id, producer, model, regnr, vin, odo, body, engine, productiondate, description, isreplacementvehicle, introducedat)
                    VALUES (@Id, @Producer, @Model, @RegNr, @Vin, @Odo, 'Hatchback', 'benzyna', @ProductionDate, @Description, true, CURRENT_TIMESTAMP)",
                    new
                    {
                        Id = id,
                        vehicle.Producer,
        ...
        private List<Guid> CreateWorks(List<Guid> clients, List<Guid> vehicles, DemoEmployees employees)
        {
            ...
            for (var i = 0; i < statuses.Length; i++)
            {
                ...
                var hasClaimNumber = i % 7 != 0;
                var hasInsurer = i % 8 != 1;
                ...
                var settled = i is 18 or 19;
                var partiallySettled = i is 14 or 17;
        ...
        private void CreateDocuments(List<Guid> workIds, Guid adminId)
        {
            var categories = new[]
            {
                "vehicle_photos", "audanet_estimates", "audatex_estimates", "insurer_decisions", "claim_assignments",
                "authorizations", "invoices", "notes", "transfer_confirmations", "other"
            };
                ...
                for (var j = 0; j < docsToCreate; j++)
                {
                    ...
                    session.Connection.Execute(@"
                        ...
                        new
                        {
                            ...
                            WorkId = workIds[i],
                            Category = category,
                            FileName = $"appra-demo-{i + 1:D2}-{category}.pdf",
                            FileSize = content.Length,
                            Content = content,
                            UploadedOn = DateTime.UtcNow.AddDays(-(20 - i)).AddHours(j),
                            EmployeeId = adminId
                        });
                }
            }
        ...
        private void CreateReplacementRentals(List<Guid> workIds, List<Guid> replacementVehicleIds)
        {
            ...
            for (var i = 0; i < workIndexes.Length; i++)
            {
                ...
                session.Connection.Execute(@"
                    ...
                    VALUES (gen_random_uuid(), @WorkId, @VehicleId, @IssuedOn, @PlannedReturnOn, @ReturnedOn, @MileageOut, @MileageIn, @FuelOut, @FuelIn, @ConditionOut, @ConditionIn, @Notes, @Status, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)",
                    new
                    {
                        WorkId = workIds[workIndexes[i]],
                        VehicleId = replacementVehicleIds[i % replacementVehicleIds.Count],
                        IssuedOn = issuedOn,
                        PlannedReturnOn = plannedReturn,
                        ReturnedOn = returnedOn,
                        MileageOut = 30000 + i * 1500,
                        MileageIn = returnedOn.HasValue ? 30250 + i * 1500 : (int?)null,
                        FuelOut = i % 2 == 0 ? "3/4" : "1/2",
                        FuelIn = returnedOn.HasValue ? "1/2" : null,
                        ConditionOut = "Stan dobry, dokumentacja zdjęciowa wykonana.",
                        ConditionIn = returnedOn.HasValue ? "Zwrot bez nowych uszkodzeń." : null,
                        Notes = i == 1 ? "Pojazd wydany bez daty zwrotu - alert demonstracyjny." : "Najem pojazdu zastępczego APPRA demo.",
                        Status = status
                    });
            }
        }
            ...
            for (var i = 0; i < workIds.Count; i++)
            {
                ...
                for (var j = 0; j < lines; j++)
                {
                    ...
                    session.Connection.Execute(@"
                        ...
                        VALUES (uuid_generate_v4(), @WorkId, @PartName, @OemNumber, @Supplier, @Quantity, @NetPrice, @VatAmount, @GrossPrice, @Status, @OrderedOn, @PlannedDeliveryOn, @DeliveredOn, @OrderNumber, @Notes, @ExternalSupplierId, @ExternalOrderId, 'manual', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)",
                        new
                        {
                            WorkId = workIds[i],
                            PartName = parts[(i + j) % parts.Length],
                            OemNumber = $"OEM-APPRA-{i + 1:D2}{j + 1:D2}",
                            Supplier = j % 2 == 0 ? "Inter Cars" : "Lokalny dostawca części",
                            Quantity = 1 + j,
                            NetPrice = net,
                            VatAmount = vat,
                            GrossPrice = net + vat,
                            Status = status,
                            OrderedOn = status == "to_order" ? (DateTime?)null : DateTime.UtcNow.AddDays(-6 + j),
                            PlannedDeliveryOn = i == 8 ? DateTime.UtcNow.AddDays(-2) : DateTime.UtcNow.AddDays(2 + j),
                            DeliveredOn = status == "delivered" || status == "returned" ? DateTime.UtcNow.AddDays(-1) : (DateTime?)null,
                            OrderNumber = status == "to_order" ? null : $"IC/APPRA/{i + 1:D2}/{j + 1}",
                            Notes = i == 8 ? "Dostawa opóźniona - alert demonstracyjny." : "Pozycja części demonstracyjnych.",
                            ExternalSupplierId = "APPRA-DEMO-SUPPLIER",
                            ExternalOrderId = status == "to_order" ? null : $"APPRA-DEMO-ORDER-{i + 1:D2}{j + 1:D2}"
                        });
                }
            }
        }
        ...
        private void CreateTasks(List<Guid> workIds, DemoEmployees employees)
        {
            var tasks = new[]
            {
                new DemoTask("Oględziny pojazdu", "inspection", employees.Expert),
                new DemoTask("Przygotować kosztorys", "estimate", employees.Expert),
                new DemoTask("Zamówić części", "parts", employees.Office),
                new DemoTask("Naprawa blacharska", "body_shop", employees.BodyWorker),
                new DemoTask("Lakierowanie elementów", "paint_shop", employees.Painter),
                new DemoTask("Kontrola jakości", "quality_control", employees.Manager),
                new DemoTask("Przygotowanie do wydania", "vehicle_release", employees.Office),
                new DemoTask("Rozliczenie sprawy", "settlement", employees.Office),
                new DemoTask("Kontakt z klientem", "office", employees.Office),
                new DemoTask("Kontakt z ubezpieczycielem", "other", employees.Expert)
            };
            ...
            for (var i = 0; i < workIds.Count; i++)
            {
                for (var j = 0; j < 2; j++)
                {
                    ...
                    var status = statuses[(i + j) % statuses.Length];
                    var dueOn = i % 5 == 0 ? DateTime.UtcNow.AddDays(-2) : DateTime.UtcNow.AddDays(2 + j);
                    session.Connection.Execute(@"
                        ...
                        VALUES (uuid_generate_v4(), @WorkId, @Title, @Description, @TaskType, @AssignedEmployeeId, @Status, @Priority, @DueOn, @CompletedOn, @Comment, @CreatedByEmployeeId, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)",
                        new
                        {
                            WorkId = workIds[i],
                            task.Title,
                            Description = "Zadanie demonstracyjne APPRA.",
                            TaskType = task.Type,
                            AssignedEmployeeId = i % 9 == 0 ? (Guid?)null : task.EmployeeId,
                            Status = status,
                            Priority = priorities[(i + j) % priorities.Length],
                            DueOn = dueOn,
                            CompletedOn = status == "completed" ? DateTime.UtcNow.AddDays(-1) : (DateTime?)null,
                            Comment = i % 5 == 0 ? "Termin zadania przekroczony - alert demonstracyjny." : "Bez uwag.",
                            CreatedByEmployeeId = employees.Admin
                        });
                }
            }
        }
        ...
        private void CreateChecklist(List<Guid> workIds, DemoEmployees employees)
        {
            var defaults = new[]
            {
                new ChecklistSeed("vehicle_photos_done", "vehicle_intake", "Zdjęcia pojazdu wykonane", 10),
                new ChecklistSeed("client_documents_added", "documents", "Dokumenty klienta dodane", 20),
                new ChecklistSeed("assignment_checked", "documents", "Cesja sprawdzona", 30),
                new ChecklistSeed("estimate_checked", "inspection", "Kosztorys sprawdzony", 40),
                new ChecklistSeed("parts_ordered", "parts", "Części zamówione", 50),
                new ChecklistSeed("parts_delivered", "parts", "Części dostarczone", 60),
                new ChecklistSeed("repair_completed", "mechanical_repair", "Naprawa zakończona", 70),
                new ChecklistSeed("fitment_checked", "body_repair", "Kontrola spasowania", 80),
                new ChecklistSeed("paint_checked", "painting", "Kontrola lakieru", 90),
                new ChecklistSeed("electronics_checked", "final_control", "Kontrola elektroniki", 100),
                new ChecklistSeed("test_drive_done", "final_control", "Jazda próbna", 110),
                new ChecklistSeed("vehicle_washed", "washing", "Pojazd umyty", 120),
                new ChecklistSeed("release_documents_ready", "vehicle_release", "Dokumenty wydania przygotowane", 130)
            };
            ...
            for (var i = 0; i < workIds.Count; i++)
            {
                ...
                for (var j = 0; j < defaults.Length; j++)
                {
                    ...
                    session.Connection.Execute(@"
                        ...
                        VALUES (uuid_generate_v4(), @WorkId, @ItemKey, @GroupKey, @ItemName, @Description, @IsCompleted, @CompletedByEmployeeId, @CompletedOn, @Notes, @SortOrder, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
                        ...
                        new
                        {
                            ...
                            item.GroupKey,
                            item.ItemName,
                            Description = "Pozycja checklisty demonstracyjnej APPRA.",
                            IsCompleted = isCompleted,
                            CompletedByEmployeeId = isCompleted ? employees.Manager : (Guid?)null,
                            CompletedOn = isCompleted ? DateTime.UtcNow.AddDays(-1) : (DateTime?)null,
                            Notes = isCompleted ? "Wykonano w danych demonstracyjnych." : "Do wykonania.",
                            item.SortOrder
                        });
                }
            }
            ...
            for (var i = 0; i < workIds.Count; i++)
            {
                for (var j = 0; j < 2; j++)
                {
                    ...
                    session.Connection.Execute(@"
                        ...
                        VALUES (gen_random_uuid(), @WorkId, @Category, @Subject, @Note, @Status, NULL, @AuthorId, @AuthorName, @OccurredOn, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, @Channel, NULL, NULL)",
                        new
                        {
                            WorkId = workIds[i],
                            Category = category,
                            Subject = category.Contains("insurer") ? "Kontakt z ubezpieczycielem" : "Kontakt w sprawie szkody",
                            Note = status == "waiting_for_response"
                                ? "Oczekujemy na odpowiedź - alert demonstracyjny."
                                : "Wpis komunikacji demonstracyjnej APPRA.",
                            Status = status,
                            AuthorId = employees.Office,
                            AuthorName = "APPRA Demo",
                            OccurredOn = DateTime.UtcNow.AddDays(-(10 - j)).AddHours(i),
                            Channel = category == "email" ? "email_placeholder" : "manual"
                        });
                }
            }
        }
        ...
        private void CreateVehicleReleases(List<Guid> workIds, DemoEmployees employees)
        {
            ...
            for (var i = 16; i < workIds.Count; i++)
            {
                ...
                session.Connection.Execute(@"
                    ...
                    VALUES (uuid_generate_v4(), @WorkId, @PlannedReleaseOn, @ReleasedOn, @ReleasedByEmployeeId, @ReceivedByName, @IdentityDocumentNumber, @MileageOut, @FuelOut, @ReleaseNotes, @ClientReceivedDocuments, @ClientReceivedInvoiceInfo, @VehicleWashed, @FinalControlCompleted, @ClientSignaturePlaceholder, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
                    ON CONFLICT (workid) DO NOTHING",
                    new
                    {
                        WorkId = workIds[i],
                        PlannedReleaseOn = DateTime.UtcNow.Date.AddDays(i == 16 ? -2 : 1),
                        ReleasedOn = released ? DateTime.UtcNow.Date.AddDays(-1) : (DateTime?)null,
                        ReleasedByEmployeeId = employees.Manager,
                        ReceivedByName = released ? "Klient demonstracyjny" : null,
                        IdentityDocumentNumber = released ? "DOW-APPRA-DEMO" : null,
                        MileageOut = 65000 + i * 1200,
                        FuelOut = "3/4",
                        ReleaseNotes = released ? "Pojazd wydany klientowi w danych demonstracyjnych." : "Pojazd przygotowany do wydania.",
                        ClientReceivedDocuments = released,
                        ClientReceivedInvoiceInfo = i != 16,
                        VehicleWashed = i != 17,
                        FinalControlCompleted = i >= 18,
                        ClientSignaturePlaceholder = "Podpis klienta zbierany poza systemem."
                    });
            }
        }
